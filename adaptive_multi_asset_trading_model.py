"""
Adaptive Multi-Asset Trading Research Model
===========================================

A Python research framework for testing systematic trading logic across
crypto, equities, and futures.

The project focuses on practical strategy-building components:
- market data loading and validation
- regime-aware signal generation
- ATR-based risk sizing and exits
- class-level risk budgets and drawdown controls
- backtesting metrics and trade accounting
- optional LLM sentiment/event checks in live mode
- optional ML expectancy modeling after enough trade history

Project notes
-------------
This started as a personal quant-finance research project. The main lesson from
building it is that strategy quality is not only about entries. Data quality,
position sizing, correlation, transaction costs, exit rules, and drawdown control
can matter as much as the initial signal.

Known limitations
-----------------
- Backtest results are sensitive to assumptions around data quality, fills,
  slippage, and transaction costs.
- Futures execution is advisory-only in the Alpaca integration because Alpaca
  does not support futures orders.
- The optional LLM sentiment/event layer is experimental and disabled unless an
  API key is supplied through environment variables.
- The codebase is still mostly contained in one large file; splitting data,
  signals, risk, execution, and reporting into modules would make maintenance
  easier.
- This project should be treated as research code, not financial advice or a
  production trading system.

Future improvements
-------------------
- Split the model into separate modules for data, signals, risk, execution, and
  reporting.
- Add unit tests for price validation, position sizing, exits, and performance
  accounting.
- Add walk-forward and out-of-sample validation workflows.
- Improve reporting with cleaner charts and summary tables.
- Add a small example notebook for running a basic backtest from scratch.
